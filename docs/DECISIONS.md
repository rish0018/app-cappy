...
- **2026-07-30   Apple Sign-In removed from v1 (reversal of the 2026-07-17 decision).** No active Apple Developer Program membership ($99/year, required for Sign-In with Apple configuration) — Google OAuth alone ships for v1. Removed: `signInWithAppleIdToken` (`packages/api`), `"apple"` from `OAuthProvider` (`packages/types`) and both `SSOButton` components, the native `expo-apple-authentication` flow + `isAppleCancellation` helper and its dependency/app.json plugin registration from `apps/mobile`, and the Apple button from both apps' Login/Signup screens. `docs/BACKEND_SSO_SETUP.md` §5 (Apple configuration) removed accordingly. Revisit if an Apple Developer Program membership is obtained later   the removal was mechanical (delete the provider branch), not a redesign, so re-adding follows the same shape as the 2026-07-17/07-21 entries above.
