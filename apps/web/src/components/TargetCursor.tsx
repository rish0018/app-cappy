...
function getContainingBlock(element: Element | null): HTMLElement | null {
...
function getContainingBlockOffset(block: HTMLElement | null): { x: number; y: number } {
...
export function TargetCursor({
  ...
  const cornersRef = React.useRef<NodeListOf<HTMLElement> | null>(null);
  const spinTl = React.useRef<gsap.core.Timeline | null>(null);
  const dotRef = React.useRef<HTMLDivElement>(null);
  const containingBlockRef = React.useRef<HTMLElement | null>(null);
  ...
  // Live-gate on fine pointer + reduced motion, per the approved scope.
  React.useEffect(() => {
    if (typeof window === "undefined" || !window.matchMedia) return;
    const pointerFineMql = window.matchMedia("(pointer: fine)");
    const reducedMotionMql = window.matchMedia("(prefers-reduced-motion: reduce)");
    const update = () => setEnabled(pointerFineMql.matches && !reducedMotionMql.matches);
    update();
    pointerFineMql.addEventListener("change", update);
    reducedMotionMql.addEventListener("change", update);
    return () => {
      pointerFineMql.removeEventListener("change", update);
      ...
      const leaveHandler = () => {
        ...
        if (cornersRef.current) {
          ...
          innerCorners.forEach((corner, index) => {
            ...
            tl.to(corner, { x: position.x, y: position.y, duration: 0.3, ease: "power3.out" }, 0);
          });
        }
