 ...
 * (task #3) instead of the hardcoded `mockWeakLetters`/`mockReviewItems`
 * fixtures LessonReview previously always showed. Falls back to the
 * caller-supplied mock set while loading, unauthenticated, unconfigured,
 * or once real data exists but nothing is actually due   so the screen
 * never renders emptier than the pre-existing mock experience.
 */
