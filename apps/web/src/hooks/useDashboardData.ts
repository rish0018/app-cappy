 ...
 * packages/types' LETTER_GROUPS/MORSE_GROUPS (see mockData.ts/
 * morseMockData.ts) -- this hook only fetches the per-user data that
 * actually changes per learner: display name, active-lesson completion,
 * letter mastery, weekly activity minutes, and a Morse level-1 mastery
 * preview. Falls back to null (screen keeps its existing mock) while
 * loading, unauthenticated, or unconfigured, matching useReviewSet/
 * useAdaptiveRecommendation's pattern.
 */
