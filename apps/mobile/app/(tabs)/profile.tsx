  ...
  return (
    <SafeAreaView className="flex-1 bg-neutral-50" edges={["top"]}>
      <ScrollView className="flex-1 px-lg" contentContainerStyle={{ paddingBottom: 32 }}>
        <Text className="mb-lg mt-md text-2xl font-bold text-neutral-800">Profile</Text>
        ...
        <Card className="mb-lg items-center">
          ...
          <Text className="text-xl font-bold text-neutral-800">{mockUser.displayName}</Text>
          <Text className="text-sm text-neutral-500">{mockUser.email}</Text>
          <View className="mt-lg w-full flex-row justify-around">
            <View className="items-center">
              <Text className="text-lg font-bold text-primary-600">{mockUser.totalXp}</Text>
              <Text className="text-xs text-neutral-500">Total XP</Text>
            </View>
            <View className="items-center">
              <Text className="text-lg font-bold text-accent-600">{mockStreak.currentStreak}</Text>
              <Text className="text-xs text-neutral-500">Day streak</Text>
            </View>
            <View className="items-center">
              <Text className="text-lg font-bold text-info-500">{mockStreak.longestStreak}</Text>
        ...
        <Card className="mb-lg">
          <Text className="mb-sm text-sm font-semibold text-neutral-500">Overall letter mastery</Text>
          <ProgressBar percentage={overallMastery} accessibilityLabel={`Overall mastery ${overallMastery}%`} />
          <Text className="mt-sm text-xs text-neutral-500">{overallMastery}% across all 26 letters</Text>
        </Card>

        <Card className="mb-lg">
          <Text className="mb-md text-sm font-semibold text-neutral-500">Mastery by group</Text>
          {mockLetterGroups.map((group) => {
            const scores = group.letters.map((letter) => mockLetterMastery[letter].masteryScore);
            const avg = Math.round(scores.reduce((a, b) => a + b, 0) / scores.length);
            return (
              <View key={group.id} className="mb-md">
                <View className="mb-xs flex-row justify-between">
                  <Text className="text-sm text-neutral-700">{group.label}</Text>
                  <Text className="text-sm text-neutral-500">{avg}%</Text>
                </View>
                <ProgressBar percentage={avg} accessibilityLabel={`${group.label} mastery ${avg}%`} />
              </View>
            );
          })}
        </Card>
        ...
        <Card className="mb-lg">
          <Text className="mb-md text-base font-bold text-neutral-800">Accessibility settings</Text>

          <View className="mb-lg flex-row items-center justify-between">
            <View className="flex-1 pr-md">
              <Text className="text-sm font-semibold text-neutral-700">Reduced motion</Text>
              <Text className="text-xs text-neutral-500">Minimize animations and celebration effects.</Text>
            </View>
            <Switch
              value={reducedMotion}
          ...
          <Text className="mb-sm text-sm font-semibold text-neutral-700">Text size</Text>
            ...
            {TEXT_SIZES.map((size) => (
              <Pressable
                ...
                className={`min-h-[44px] flex-1 items-center justify-center rounded-md border ${
                  textSize === size ? "border-primary-500 bg-primary-500" : "border-neutral-200 bg-white"
                }`}
              >
                <Text className={`text-sm font-semibold capitalize ${textSize === size ? "text-white" : "text-neutral-600"}`}>
                  {size}
                </Text>
              </Pressable>
            ))}
